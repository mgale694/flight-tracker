...
class Waveshare213bV4(DisplayImpl):
    ...
    def initialize(self):
        ...
        try:
            from raspi.ui.hw.libs.waveshare.v213inb_v4.epd2in13b_V4 import EPD

            self._display = EPD()
            self._display.init()
            self._display.Clear()
            logging.info("✓ Waveshare display hardware initialized successfully")
        except Exception as e:
            logging.error(f"❌ Failed to initialize Waveshare hardware: {e}")
            logging.warning(
                "Using mock display functionality - hardware output disabled"
            )
            self._display = None

    def render(self, canvasBlack=None, canvasRed=None):
        if self._display is None:
            logging.debug("Mock render: display hardware not available")
            return
        ...
        try:
            ...
            logging.error(f"Error during display render: {e}")

    def clear(self):
        if self._display is None:
            logging.debug("Mock clear: display hardware not available")
            return
