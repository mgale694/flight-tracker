...
class WaveshareV1(DisplayImpl):
    ...
    def initialize(self):
        if self.config['color'] == 'black':
            ...
            from raspi.ui.hw.libs.waveshare.v1.epd2in13 import EPD
            self._display = EPD()
            ...
            from raspi.ui.hw.libs.waveshare.v1.epd2in13bcFAST import EPD
            self._display = EPD()
            self._display.init()
            self._display.Clear()
        else:
            logging.info("initializing waveshare v1 display 3-color mode")
            from raspi.ui.hw.libs.waveshare.v1.epd2in13bc import EPD
            self._display = EPD()
            self._display.init()
            self._display.Clear()
